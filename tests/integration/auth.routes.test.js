...
(0, vitest_1.describe)('POST /api/auth/login — validación', () => {
    (0, vitest_1.it)('400 VALIDATION_ERROR si falta la contraseña', async () => {
        const res = await (0, supertest_1.default)(app_1.default).post('/api/auth/login').send({ usuario: 'docente01' });
    ...
    (0, vitest_1.it)('400 VALIDATION_ERROR si falta el usuario', async () => {
        const res = await (0, supertest_1.default)(app_1.default).post('/api/auth/login').send({ contrasena: 'password123' });
        (0, vitest_1.expect)(res.status).toBe(400);
        (0, vitest_1.expect)(res.body.codigo).toBe('VALIDATION_ERROR');
    });
    ...
    (0, vitest_1.it)('400 VALIDATION_ERROR si la contraseña tiene menos de 8 caracteres', async () => {
        const res = await (0, supertest_1.default)(app_1.default)
            .post('/api/auth/login')
...
(0, vitest_1.describe)('POST /api/auth/login — autenticación', () => {
    ...
    (0, vitest_1.it)('401 UNAUTHORIZED si el usuario está inactivo', async () => {
        const { prisma } = await Promise.resolve().then(() => __importStar(require('@/config/database.config')));
...
(0, vitest_1.describe)('POST /api/auth/refresh — rotación', () => {
    ...
    (0, vitest_1.it)('401 si el hash del token no está en BD (ya girado o fabricado)', async () => {
        ...
        const tokenValido = jwt.sign({ id_ct_usuario: 1, usuario: 'test', email: null, rol: 'CAJERO' }, process.env['JWT_REFRESH_SECRET'], { expiresIn: '7d' });
        const res = await (0, supertest_1.default)(app_1.default)
            .post('/api/auth/refresh')
    ...
    (0, vitest_1.it)('401 si el token ya fue revocado (reutilización detectada) → invalida familia', async () => {
        const { prisma } = await Promise.resolve().then(() => __importStar(require('@/config/database.config')));
        ...
        (0, vitest_1.expect)(res.status).toBe(401);
        (0, vitest_1.expect)(res.body.codigo).toBe('UNAUTHORIZED');
        (0, vitest_1.expect)(vitest_1.vi.mocked(prisma.dt_refresh_token.updateMany)).toHaveBeenCalledWith(vitest_1.expect.objectContaining({
