...
describe('Módulo de Configuración — Rutas de Integración', () => {
  ...
  describe('GET /api/configuracion', () => {
    ...
    it('debe crear configuración por defecto si no existe', async () => {
      ...
      vi.mocked(prisma.ct_configuracion.create).mockResolvedValue({ id_ct_configuracion: 1, nombre_restaurante: 'Mi Restaurante' } as any);
  ...
  describe('PATCH /api/configuracion', () => {
    it('debe actualizar la configuración', async () => {
      vi.mocked(prisma.ct_configuracion.findFirst).mockResolvedValue({ id_ct_configuracion: 1 } as any);
      vi.mocked(prisma.ct_configuracion.update).mockResolvedValue({ id_ct_configuracion: 1, nombre_restaurante: 'Nuevo Nombre' } as any);
