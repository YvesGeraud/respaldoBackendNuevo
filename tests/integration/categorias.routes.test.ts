...
describe('Módulo de Categorías — Rutas de Integración', () => {
  ...
  describe('GET /api/categorias', () => {
    it('debe retornar lista de categorias', async () => {
      const mockCategorias = [
        { id_ct_categoria: 1, nombre: 'Postres', estado: true },
      ];
  ...
  describe('PATCH /api/categorias/:id', () => {
    it('debe actualizar una categoría', async () => {
      ...
      vi.mocked(prisma.ct_categoria.update).mockResolvedValue({ id_ct_categoria: 1, nombre: 'Editada' } as any);
  ...
  describe('DELETE /api/categorias/:id', () => {
    it('debe desactivar una categoría', async () => {
      vi.mocked(prisma.ct_categoria.findUnique).mockResolvedValue({ id_ct_categoria: 1, estado: true } as any);
      vi.mocked(prisma.ct_categoria.update).mockResolvedValue({ id_ct_categoria: 1, estado: false } as any);
